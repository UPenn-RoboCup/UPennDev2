...
local dsp_ratio = parameters.dsp_ratio or 0.25
...
local function zsin(theta, alpha)
  -- generates a sinusoidal waveform with extended zero crossings
  -- alpha in [0, 1] determines the percentage of deadzone in a cycle
  -- where zsin(theta, 0) = sin(theta)
...
local function zcos(theta, alpha)
  return zsin(theta + math.pi/2, alpha) 
end
...
function walk:update()
  if walk.active then
    if is_walking then
      ...
      l_foot_pos[1] = x_offset + x_swing - velocity[1]*zcos(ph, dsp_ratio)
      l_foot_pos[2] = y_offset + y_swing - velocity[2]*zcos(ph, dsp_ratio)
      l_foot_pos[3] = z_offset - z_swing + step_amplitude*zsin(ph, dsp_ratio)
      l_foot_pos[6] = -velocity[3]*zcos(ph, dsp_ratio)
      local r_foot_pos = vector.zeros(6) 
      r_foot_pos[1] = x_offset + x_swing + velocity[1]*zcos(ph, dsp_ratio)
      r_foot_pos[2] =-y_offset + y_swing + velocity[2]*zcos(ph, dsp_ratio)
      r_foot_pos[3] = z_offset - z_swing - step_amplitude*zsin(ph, dsp_ratio)
      r_foot_pos[6] = velocity[3]*zcos(ph, dsp_ratio)
