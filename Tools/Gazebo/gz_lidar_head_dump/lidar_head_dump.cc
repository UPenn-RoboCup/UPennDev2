...
void LidarHeadDump::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf) {
  ...
  assert(csocket);
  int rc2 = zmq_bind(csocket, "ipc:///tmp/lidar0");
  assert(rc2==0);
}

// Update the controller
void LidarHeadDump::OnNewLaserScans()
{
  struct timeval t;
  gettimeofday(&t, NULL);
