...
static int forward_joints(lua_State *L)
{
	...
	std::vector<double> q = YouBot_kinematics_forward_joints(&r[0]);
	lua_pushvector(L, q);
	return 1;
}
...
static int forward_head(lua_State *L) {
	...
	Transform t = YouBot_kinematics_forward_head(&q[0]);
...
static int forward_l_arm(lua_State *L) {
	std::vector<double> q = lua_checkvector(L, 1);
	Transform t = YouBot_kinematics_forward_l_arm(&q[0]);
	lua_pushtransform(L, t);
	return 1;
}
...
static int forward_r_arm(lua_State *L) {
	std::vector<double> q = lua_checkvector(L, 1);
	Transform t = YouBot_kinematics_forward_r_arm(&q[0]);
	lua_pushtransform(L, t);
	return 1;
}
*/
...
static int forward_l_leg(lua_State *L) {
	std::vector<double> q = lua_checkvector(L, 1);
	Transform t = YouBot_kinematics_forward_l_leg(&q[0]);
	lua_pushtransform(L, t);
	return 1;
}
...
static int forward_r_leg(lua_State *L) {
	std::vector<double> q = lua_checkvector(L, 1);
	Transform t = YouBot_kinematics_forward_r_leg(&q[0]);
	lua_pushtransform(L, t);
	return 1;
}
...
static int l_arm_torso_7(lua_State *L) {
	...
	Transform t = YouBot_kinematics_forward_l_arm_7(&q[0],bodyPitch,&qWaist[0],
...
static int r_arm_torso_7(lua_State *L) {
	...
	Transform t = YouBot_kinematics_forward_r_arm_7(&q[0],bodyPitch,&qWaist[0], 
		handOffsetXNew, handOffsetYNew, handOffsetZNew);
	lua_pushvector(L, position6D(t));
	return 1;
}
...
static int l_wrist_torso(lua_State *L) {
	...
	Transform t = YouBot_kinematics_forward_l_wrist(&q[0],bodyPitch,&qWaist[0]);
	lua_pushvector(L, position6D(t));	
	return 1;
}
...
static int r_wrist_torso(lua_State *L) {
	...
	Transform t = YouBot_kinematics_forward_r_wrist(&q[0],bodyPitch,&qWaist[0]);
	lua_pushvector(L, position6D(t));	
	return 1;
}
...
static int inverse_l_arm_7(lua_State *L) {
	...
	qArm = YouBot_kinematics_inverse_l_arm_7(trArm,&qArmOrg[0],shoulderYaw,bodyPitch,&qWaist[0],
...
//Now we can use custom hand x/y/z offset (for claws)
	...
	Transform trArm = transform6D(&pArm[0]);
	qArm = YouBot_kinematics_inverse_r_arm_7(trArm,&qArmOrg[0],shoulderYaw,bodyPitch,&qWaist[0],
		handOffsetXNew, handOffsetYNew, handOffsetZNew, flip_shoulderroll);
	lua_pushvector(L, qArm);
	return 1;
}
...
static int inverse_l_wrist(lua_State *L) {
	...
	Transform trArm = transform6D(&pArm[0]);
	qArm = YouBot_kinematics_inverse_l_wrist(trArm,&qArmOrg[0],shoulderYaw,bodyPitch,&qWaist[0]);
	lua_pushvector(L, qArm);
	return 1;
}
...
static int inverse_r_wrist(lua_State *L) {
	...
	Transform trArm = transform6D(&pArm[0]);
	qArm = YouBot_kinematics_inverse_r_wrist(trArm,&qArmOrg[0],shoulderYaw,bodyPitch,&qWaist[0]);
	lua_pushvector(L, qArm);
	return 1;
}
...
static int inverse_arm_given_wrist(lua_State *L) {
	...
	Transform trArm = transform6D(&pArm[0]);	
	qArm = YouBot_kinematics_inverse_arm_given_wrist(trArm,&qArmOrg[0],bodyPitch,&qWaist[0]);
	lua_pushvector(L, qArm);
	return 1;
}
...
static int torso_l_leg(lua_State *L) {
	std::vector<double> q = lua_checkvector(L, 1);
	Transform t = inv(YouBot_kinematics_forward_l_leg(&q[0]));
	lua_pushvector(L, position6D(t));
	return 1;
}
...
static int torso_r_leg(lua_State *L) {
	std::vector<double> q = lua_checkvector(L, 1);
	Transform t = inv(YouBot_kinematics_forward_r_leg(&q[0]));
	lua_pushvector(L, position6D(t));
	return 1;
}
...
static int inverse_joints(lua_State *L)
{
	...
	std::vector<double> q = lua_checkvector(L, 1);
	std::vector<double> r = YouBot_kinematics_inverse_joints(&q[0]);
	lua_pushvector(L, r);
	return 1;
}
...
static int com_upperbody(lua_State *L) {
	...
	std::vector<double> r = YouBot_kinematics_com_upperbody(
...
static int calculate_support_torque(lua_State *L) {
	...
	std::vector<double> r = YouBot_kinematics_calculate_support_torque(
...
static int calculate_knee_height(lua_State *L) {
	...
	double trKnee = YouBot_kinematics_calculate_knee_height(&qLeg[0]);
...
static int inverse_l_leg(lua_State *L) {
	...
	qLeg = YouBot_kinematics_inverse_r_leg(trLeg);
...
static int inverse_r_leg(lua_State *L) {
	...
	std::vector<double> pLeg = lua_checkvector(L, 1);
	Transform trLeg = transform6D(&pLeg[0]);
	qLeg = YouBot_kinematics_inverse_l_leg(trLeg);
	lua_pushvector(L, qLeg);
	return 1;
}
...
static int inverse_legs(lua_State *L) {
	...
	qLLeg = YouBot_kinematics_inverse_l_leg(trTorso_LLeg);
	qRLeg = YouBot_kinematics_inverse_r_leg(trTorso_RLeg);
