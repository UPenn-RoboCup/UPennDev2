...
const double MIN_ERROR_DISTANCE = 50;//200
//const double ERROR_DEPTH_FACTOR = 0.10;
const double ERROR_DEPTH_FACTOR = 0.08; //.12
//const double ERROR_ANGLE = 5*PI/180;
const double ERROR_ANGLE = 3*PI/180;//7
...
// Check input values
  ...
  bm.BallObservation(objGaussian, (int)(uncertainty[0]));
  

  // Send the output
  //double nval = *uncertainty;
  //plhs[0] = mxCreateDoubleScalar(x);

  plhs[0] = mxCreateDoubleMatrix(1, 4, mxREAL);
  double* vals = mxGetPr(plhs[0]);
  bm.getBall( vals[0], vals[1], vals[2], vals[3]);
