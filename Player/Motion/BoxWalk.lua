...
stance0,stance1,stance_changing=0,0,0;
...
function entry()
  ...
  uLeft = util.pose_global(
	vector.new({-supportX, 0, 0})+uLRFootOffset,uTorso);
  uRight = util.pose_global(
	vector.new({-supportX, 0, 0})-uLRFootOffset,uTorso);
...
function update()
  if (not active and walkKickRequest==0) then 
    update_still();
    return; 
  end
