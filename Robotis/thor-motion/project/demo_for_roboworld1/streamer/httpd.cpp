...
int y_amplitude = 0;
int a_amplitude = 0;
const int steptime = 400000;


void ReduceStepSize(bool xstep, bool ystep)
{
	int xsign = 0;
	int ysign = 0;

	if( ((int) (Walking::GetInstance()->X_MOVE_AMPLITUDE)) > 0 )
		xsign = 1;
	else if(((int) (Walking::GetInstance()->X_MOVE_AMPLITUDE)) < 0 )
		xsign = -1;
	else
		xsign = 0;

	if( ((int) (Walking::GetInstance()->Y_MOVE_AMPLITUDE)) > 0 )
		ysign = 1;
	else if(((int) (Walking::GetInstance()->Y_MOVE_AMPLITUDE)) < 0 )
		ysign = -1;
	else
		ysign = 0;
	...
	while(true)
	{
		xnum = abs(Walking::GetInstance()->X_MOVE_AMPLITUDE) / 5;
		...
		if(xstep)
		{
			if( xnum == 0 )
				...
				Walking::GetInstance()->X_MOVE_AMPLITUDE -= xsign*5;

			ynum = abs(Walking::GetInstance()->Y_MOVE_AMPLITUDE) / 5;
		}
		...
		if(ystep)
		{
			if( ynum == 0 )
				...
				Walking::GetInstance()->Y_MOVE_AMPLITUDE -= ysign*5;
...
EnableList EnableListforDemo[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS];
...
void httpd::input_cmd(in_cmd_type cmd, float value, char* res_str)
{
    ...
    switch(cmd) {
    case IN_CMD_RELOAD:
        Thor::Camera::GetInstance()->LoadINISettings(ini);
        ...
        break;
    case IN_CMD_SAVE:
        Thor::Camera::GetInstance()->SaveINISettings(ini);
        if(finder == NULL) return;
        if(finder->color_section == "")
    ...
    case IN_CMD_GAIN_PLUS:
        res = Thor::Camera::GetInstance()->v4l2GetControl(V4L2_CID_GAIN);
        sprintf(res_str, "%d", res);
        if(res < 255)
            Thor::Camera::GetInstance()->v4l2SetControl(V4L2_CID_GAIN, res+(int)value);
        sprintf(res_str, "%d", Thor::Camera::GetInstance()->v4l2GetControl(V4L2_CID_GAIN));
        break;
    ...
    case IN_CMD_EXPOSURE_PLUS:
        res = Thor::Camera::GetInstance()->v4l2GetControl(V4L2_CID_EXPOSURE_ABSOLUTE);
        sprintf(res_str, "%d", res);
        if(res < 10000)
            Thor::Camera::GetInstance()->v4l2SetControl(V4L2_CID_EXPOSURE_ABSOLUTE, res+(int)value);
        sprintf(res_str, "%d", Thor::Camera::GetInstance()->v4l2GetControl(V4L2_CID_EXPOSURE_ABSOLUTE));
        break;
    ...
    case IN_CMD_WALK_MODE:
        if(value == 1)
        {
        	...
        	else
        	{
            	...
            	for(int i =0; i < 14; i++)
            	{
            		EnableListforDemo[i].uID = "Action";
            	}

            	EnableListforDemo[26].uID = "Action";
            	EnableListforDemo[27].uID = "Action";
            	EnableListforDemo[28].uID = "Test";
            	EnableListforDemo[29].uID = "Test";

        		Action::GetInstance()->Start(19, EnableListforDemo);
        		while(Action::GetInstance()->IsRunning())
        			usleep(8000);
        		...
        		for(int i = 0; i < MotionStatus::MAXIMUM_NUMBER_OF_JOINTS; i++)
        			MotionStatus::m_EnableList[i].uID = "Walking";

        		MotionStatus::m_EnableList[28].uID = "Test";
        		MotionStatus::m_EnableList[29].uID = "Test";
        ...
        else
        {
        	...
        	if( ((int) (Walking::GetInstance()->A_MOVE_AMPLITUDE)) > 0 )
        		asign = 1;
        	else if(((int) (Walking::GetInstance()->A_MOVE_AMPLITUDE)) < 0 )
        		asign = -1;
        	else
        		asign = 0;
    		...
    		{
    			xnum = abs(Walking::GetInstance()->X_MOVE_AMPLITUDE) / 5;
    			anum = abs(Walking::GetInstance()->A_MOVE_AMPLITUDE) / 5;
    			...
    			if( anum == 0 )
    				...
    				Walking::GetInstance()->A_MOVE_AMPLITUDE -= asign*5;
    ...
    case IN_CMD_PLAY_PAGE:
    	...
    	if((((int) value) != 23 ) && (((int) value) != 28) && (((int) value) != 27 ) && (((int) value) != 11 ) )
    	{
    		fprintf(stderr, "Can not play Page %d\n",(int)value);
    		break;
    	}
    	if(Action::GetInstance()->IsRunning())
    		Action::GetInstance()->Stop();
    	...
    	EnableListforDemo[26].uID = "Action";
    	EnableListforDemo[27].uID = "Action";
    	EnableListforDemo[28].uID = "Action";
    	EnableListforDemo[29].uID = "Action";
    	fprintf(stderr, "Start Page %d\n",(int)value);
    	if( ((int) value) == 2)
    	{
    		Action::GetInstance()->Start((int)value);
    		while(Action::GetInstance()->IsRunning())
    			usleep(8000);
    	...
    	else
    	{
        	...
        	EnableListforDemo[14].uID = "Walking";
        	EnableListforDemo[15].uID = "Walking";
        	EnableListforDemo[16].uID = "Walking";
        	EnableListforDemo[17].uID = "Walking";
        	EnableListforDemo[18].uID = "Walking";
        	EnableListforDemo[19].uID = "Walking";
        	EnableListforDemo[20].uID = "Walking";
        	EnableListforDemo[21].uID = "Walking";
        	EnableListforDemo[22].uID = "Walking";
        	EnableListforDemo[23].uID = "Walking";
        	EnableListforDemo[24].uID = "Walking";
        	EnableListforDemo[25].uID = "Walking";

    		Action::GetInstance()->Start((int)value, EnableListforDemo);

    		while(Action::GetInstance()->IsRunning())
    			usleep(8000);
    	}
    	break;
    case IN_CMD_WALK_X_AMPLITUDE:
    	x_amplitude += value;
    	if(x_amplitude >= 40)
    		x_amplitude = 40;
    	else if(x_amplitude <= 0)
    		x_amplitude = 0;
    	fprintf(stderr, "walk x amp %d\n", x_amplitude);
    	sprintf(res_str, "%d", x_amplitude);
    	break;
    case IN_CMD_WALK_A_AMPLITUDE:
    	a_amplitude += value;
    	if(a_amplitude >= 40)
    		a_amplitude = 40;
    	else if(a_amplitude <= 0)
    		a_amplitude = 0;
    	fprintf(stderr, "walk a amp %d\n", a_amplitude);
    	sprintf(res_str, "%d", a_amplitude);
    	break;
    case IN_CMD_WALK_GO:
    	if(value == 0)
    	{
    		...
    		fprintf(stderr, "All Motion Stop\n");
    	}
    	else if(value == 8)
    	{
//        	Walking::GetInstance()->A_MOVE_AMPLITUDE  = 0;
...
//
//        	Walking::GetInstance()->Y_MOVE_AMPLITUDE  = 0;
        	if(Walking::GetInstance()->X_MOVE_AMPLITUDE + 10 > 50)
        		Walking::GetInstance()->X_MOVE_AMPLITUDE  = 50;
        	else
        		Walking::GetInstance()->X_MOVE_AMPLITUDE += 10;

    		fprintf(stderr, "walk forward\n");
    	}
    	else if(value == 4)
    	{
//        	Walking::GetInstance()->A_MOVE_AMPLITUDE  = 0;
...
//    		while(true)
//    		{
//    			xnum = Walking::GetInstance()->X_MOVE_AMPLITUDE / 5;
//
//    			if( xnum == 0 )
//    				break;
//    			else
...
//
//        	Walking::GetInstance()->Y_MOVE_AMPLITUDE  = 0;
        	if(Walking::GetInstance()->X_MOVE_AMPLITUDE - 5 < -50)
        		Walking::GetInstance()->X_MOVE_AMPLITUDE  = -50;
        	else
        		Walking::GetInstance()->X_MOVE_AMPLITUDE -= 10;

    		fprintf(stderr, "walk backward\n");
    	}
    	else
    		fprintf(stderr, "?????\n");
    	break;
    case IN_CMD_HEAD_PAN_ANGLE:
    	MotionStatus::m_EnableList[26].uID = "Test";
    	static int pindex = -1;
    	if(pindex == -1)
    	{
    		for(pindex = 0; pindex < MotionStatus::m_CurrentJoints.size(); pindex++)
    			if(MotionStatus::m_CurrentJoints[pindex].m_ID == 29)
    			{
    				MotionStatus::m_CurrentJoints[pindex].m_Value += value*151875.0/180.0;
    				sprintf(res_str, "%d", (int) ((MotionStatus::m_CurrentJoints[pindex].m_Value)*180.0/151875.0));
    				break;
    			}
    	}
    	else
    	...
    	break;
    case IN_CMD_HEAD_TILT_ANGLE:
    	MotionStatus::m_EnableList[29].uID = "Test";
    	static int tindex = -1;
    	if(tindex == -1)
    	{
    		for(tindex = 0; tindex < MotionStatus::m_CurrentJoints.size(); tindex++)
    			if(MotionStatus::m_CurrentJoints[tindex].m_ID == 30)
    			{
    				MotionStatus::m_CurrentJoints[tindex].m_Value += value*151875.0/180.0;
    				sprintf(res_str, "%d", (int) ((MotionStatus::m_CurrentJoints[tindex].m_Value)*180.0/151875.0));
    				break;
    			}
    	}
    	else
