...
local function check_coordinateA(centroid, scale, maxD, maxH1, maxH2,balldebug)
 ...
 local v = torch.mv(trHead, v0) / v0[4]
  if balldebug then
    debug_ball(string.format("Ball v0:%.2f %.2f %.2f\n",v[1],v[2],v[3]))
  end
...
function Vision.ball(labelA_t, labelB_t, cc_t)
  debug_ball_clear()
  ...
  for i=1,math.min(5, nProps) do
    ...
    if check_fail==false then
      debug_ball(string.format(
      'Ball detected at %.2f, %.2f (z = %.2f)',
      propsA.v[1],propsA.v[2], propsA.v[3]))
...
function Vision.goal(labelA_t, labelB_t, cc_t)
...
function Vision.goal_low(labelB_t)
  local goals = {}
  goals.detect = 0
  -- Parameters
...
function Vision.obstacle(labelB_t)
  -- Obstacle table
...
function Vision.line(labelB_t)
  ...
  local lines, line_debug = {}, ''
    ...
    for i=1, #linePropsB do
      ...
      if length>line_cfg.min_length and
        linecount<num_line and
        vHeight<vHeightMax then
        ...
        lines.angle[linecount]=math.abs(
          math.atan2(
            vendpoint[1][2]-vendpoint[2][2], vendpoint[1][1]-vendpoint[2][1])
        );
      end
    end
...
--  print("vision fps:",1/(t-t_last))
  ...
  local ball_fails, ball = Vision.ball(labelA_t, labelB_t, cc_t)
  local post_fails, posts = Vision.goal(labelA_t, labelB_t, cc_t)
  ...
  if wcm.get_obstacle_enable()==0 then
    ...
    obstacle_fails, obstacles = Vision.obstacle(labelB_t)
  end

  if gcm.get_game_role()==0 then
    --line_fails, lines = Vision.line(labelB_t)
  end
