...
bool epos_thread::check_joint_settings()
{
  ...
  bool joint_p_gain_valid = 
    (dcm.joint_p_gain[0] == dcm.joint_p_gain[1] == 
     dcm.joint_p_gain[2]) &&
    (dcm.joint_p_gain[4] == dcm.joint_p_gain[5]) &&
    (dcm.joint_p_gain[6] == dcm.joint_p_gain[7] ==
     dcm.joint_p_gain[8]) &&
    (dcm.joint_p_gain[10] == dcm.joint_p_gain[11]);
  return joint_enable_valid && joint_p_gain_valid;
}
...
void epos_thread::update_actuator_settings()
{
  ...
  for (int i = 0; i < m_id.size(); i++)
  {
    ...
    if ((int)dcm.joint_p_gain[joint_id] == 0)
    ...
    || (((int)dcm.joint_p_gain[joint_id] == 1) && (mode != -1))
    || (((int)dcm.joint_p_gain[joint_id] == 0) && (mode != -32)))
    {
      dcm.joint_p_gain_updated[joint_id] = 0; 
      switch ((int)dcm.joint_p_gain[joint_id])
      {
        case 1 :
