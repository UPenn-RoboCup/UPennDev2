...
/* Record Absolute Truth */
FILE* ball_abs_truth;
long trial_num;

// Did we get hit? when?
char gotHit;
long count; //counter incremented in step
long hitCount;
...
*/
  ...
  fscanf(ball_params,"%ld",&trial_num);
  // Webots mixes y and z
  fscanf(ball_params,"%lf %lf %lf",&bx0,&bz0,&by0);
  fscanf(ball_params,"%lf %lf %lf",&bvx0,&bvz0,&bvy0);
  fclose( ball_params );
  gotHit = 0;
  count = 0;
  hitCount = 0;
...
/*
  char buffer[15];
  sprintf(buffer, "%ld", trial_num);  
  char filename[100];
  strcpy (filename,"/Users/stephen/Desktop/dodgeball_sim/ball_abs_truth_");
  strcat (filename, buffer);
  strcat (filename, ".txt" );
*/
  char filename[100];
  sprintf(filename, "/Users/stephen/Desktop/dodgeball_sim/ball_abs_truth_%ld.txt", trial_num);

  ball_abs_truth = fopen(filename,"w");
  fprintf( ball_abs_truth, "count x y z vx vy vz\n" );
}
...
//    dBodyAddForce(ballBody, -3,0,0);
  ...
  // Record the position and velocity of the Ball
  const dReal* position = dBodyGetPosition( ballBody );
  const dReal* velocity = dBodyGetLinearVel( ballBody );

  // Webots mixes y and z
  fprintf( ball_abs_truth, "%ld %lf %lf %lf %lf %lf %lf\n", count, position[0], position[2], position[1], velocity[0], velocity[2], velocity[1] );
...
void webots_physics_cleanup() {
   ...
   */
  // Write if a collision happened or not...
  char filename[100];
  sprintf(filename, "/Users/stephen/Desktop/dodgeball_sim/ball_params_%ld.txt", trial_num);
  FILE* ball_params = fopen(filename,"w");
  fprintf(ball_params,"Trial %ld %d %ld\n", trial_num, gotHit, hitCount); // Trial - got hit or not
  // Webots mixes y and z
  fprintf(ball_params,"Position %lf %lf %lf\n", bx0,  bz0,  by0  ); // Start position
  fprintf(ball_params,"Velocity %lf %lf %lf\n", bvx0, bvz0, bvy0 ); // Star velocity
  fclose( ball_params );


  fclose( ball_abs_truth );
}
