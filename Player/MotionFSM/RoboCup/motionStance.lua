...
if hcm.get_legdebug_enable_balance()>0 then
  ...
  --Toe push up: negative y torque readings!
...
------------------------------------------------------------------------------------------------
 ...
 k_balancing_y = -100 / (math.pi/180) --10Nm per 1 deg error
  ...
  print(string.format("Y angle err: %.2f",y_err*180/math.pi))
...
--  aShiftTargetY[2] = (-balancing_torque+rt_y)*k_const_ty + avShiftY[2]*r_const_ty
  ...
  aShiftTargetY[2] = aShiftY[2]+(-balancing_torque+rt_y)*k_const_ty + avShiftY[2]*r_const_ty
