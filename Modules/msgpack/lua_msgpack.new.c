...
static int msgpack_unpack_api( lua_State *L ) {
	...
	// why this order...? I think b/c unpack_any may push multiple objects...
	// For now, though, I think we do our default method (assume just single table to be unpacked)
	//lua_pushnumber(L,-123456); // push readlen and replace it later
	// Unpack the data into L
...
static int msgpack_unpacker_feed_api( lua_State *L ) {
	...
	lua_pushnumber(L,u->size-u->used);
	return 2;
}
...
static int msgpack_createUnpacker_api( lua_State *L ) {
	size_t bufsz = luaL_optint(L,1,1024);
#if 0    
...
static const luaL_reg msgpack_f[] = {
	...
	{"unpacker", msgpack_createUnpacker_api },
	{"feed", msgpack_unpacker_feed_api },
	{"pull", msgpack_unpacker_pull_api },        
	{NULL,NULL}
};
