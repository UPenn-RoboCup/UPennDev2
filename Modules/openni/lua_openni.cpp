...
/*
	 ...
	 */
/* Deng, Xiang 2016 + rangedata conversion */
...
static int lua_stream_info(lua_State *L) {
	...
	// // Depth
	//   dmode = depth->getVideoMode();
	//  resX = dmode.getResolutionX();
	// 	resY = dmode.getResolutionY();
	//  fps = dmode.getFps();
	// lua_newtable(L);
	// lua_pushstring(L, "width");
	// lua_pushnumber(L,resX);
...
static int lua_update_rgbd(lua_State *L ) {
	...
	printf("reading...\n");
			...
			if (1) {
			// std::cout<<"H : "<<dframe->getHeight()<<std::endl;
			// cout<<"size: "<<dframe->getDataSize()<<endl;
			//void * ddata;
			//*ddate=*dframe->getData();
			//cout<<ddata[1]<<endl;
        // 获取data array
          openni::DepthPixel* pDepth= (  openni::DepthPixel*)dframe->getData();
            //openni::DepthPixel* pDepthB= new openni::DepthPixel(*pDepth) ; 
					openni::DepthPixel* pDepthB= (openni::DepthPixel*)malloc(dframe->getDataSize());
          
            //memcpy(pDepthB, pDepth, dframe->getDataSize()/2 * sizeof(openni::DepthPixel));
            //cout<<*pDepth<<endl;
            //*(pDepthB)= *(pDepth);
            //std::copy(pDepthB, pDepth,  sizeof(pDepth));
        // const openni::RGB888Pixel* pColor
        //     = (const openni::RGB888Pixel*)cframe->getData();
        //cout<<pDepthB<<" "<<pDepth<<endl;    
           int maxd=0;
        // for (int di=0;di< dframe->getDataSize()/2;di++ ){
        // 	if (maxd < pDepth[di]){
        // 		maxd= pDepth[di];
        // 		//cout<<di<<endl;
        // 	}
        // }
            ...
            for (int di=0;di< dframe->getDataSize()/2;di++ ){
            		...
            		pDepthB[di]=openni::DepthPixel(int((double)pDepth[di]/maxd*255));

            		//cout<<pDepthB[di]<<endl;        	 
            }        
            //cout<<"Max depth " <<maxd<<endl;
            
            lua_pushlightuserdata( L, (void*)(pDepthB) ); 
      }
			/*
	switch (readyStream)
	{
