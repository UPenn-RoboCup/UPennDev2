...
%% Slow Channel
figure(1);
plot(ts_ittybitty0_in - ts_ittybitty0_in(1), bs_ittybitty0_in, 'rx', ...
    ts_ittybitty1_in - ts_ittybitty1_in(1), bs_ittybitty1_in, 'bo', ...
    ts_feedback_in - ts_feedback_in(1), bs_feedback_in, 'k-', ...
    ts_node_in - ts_node_in(1), bs_node_in, 'gs' ...
    );
...
figure(2);
[h, a1, a2] = plotyy(ts_camera0_in - ts_camera0_in(1), bs_camera0_in/1e3, ...
    ts_mesh0_in - ts_mesh0_in(1), bs_mesh0_in/1e3);
...
title('Distribution of Packet Intervals (Commands)', 'FontSize', 16);
ylabel('Count');
xlabel('Successive Packet Interval (sec)'   , 'FontSize', 14);
...
figure(8);
plot([ts_node_out; ts_node_in] - ts_node_out(1), ...
    cumsum([bs_node_out; bs_node_in]), 'b.-', ...
    ts_node_out - ts_node_out(1), cumsum(bs_node_out), 'r.-');
xlim(trange - trange(1));
title('Channel Usage', 'FontSize', 16);
xlabel('Successive Packet Interval (sec)', 'FontSize', 14);
ylabel('Cumulative Commands (bytes)', 'FontSize', 14);
legend('Indoors', 'Outdoors');
