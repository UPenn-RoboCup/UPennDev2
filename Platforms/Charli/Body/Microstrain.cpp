...
#define IMU_EXCEPT(except, msg, ...) \
  { \
    char buf[1000]; \
    snprintf(buf, 1000, msg" (in Microstrain:%s)", ##__VA_ARGS__, __FUNCTION__); \
    throw except(buf); \
  }

// Some systems (e.g., OS X) require explicit externing of static class
// members.
extern const double Microstrain::G;
extern const double Microstrain::KF_K_1;
extern const double Microstrain::KF_K_2;
...
////////////////////////////////////////////////////////////////////////////////
// Constructor
Microstrain::Microstrain() : fd(-1), continuous(false)
{ }
...
// Open the IMU port
void
Microstrain::openPort(const char *port_name)
{
...
void
Microstrain::initTime(double fix_off)
{
  wraps = 0;
...
void
Microstrain::initGyros(double* bias_x, double* bias_y, double* bias_z)
{
  wraps = 0;
...
bool
Microstrain::setContinuous(cmd command)
{
  uint8_t cmd[4];
  uint8_t rep[8];
...
Microstrain::stopContinuous()
{
  ...
  if (tcflush(fd, TCIOFLUSH) != 0)
    IMU_EXCEPT(microstrain_3dmgx3_imu::Exception, "Tcflush failed");
...
void
Microstrain::receiveAccelAngrateMag(uint64_t *time, double accel[3], double angrate[3], double mag[3])
{
  int i, k;
  uint8_t rep[43];
...
void
Microstrain::receiveAccelAngrateOrientation(uint64_t *time, double accel[3], double angrate[3], double orientation[9])
{
  int i, k;
  uint8_t rep[67];
...
void
Microstrain::receiveAccelAngrate(uint64_t *time, double accel[3], double angrate[3])
{
  int i, k;
  uint8_t rep[31];
...
void
Microstrain::receiveEuler(uint64_t *time, double *roll, double *pitch, double *yaw)
{
  uint8_t rep[19];
...
bool Microstrain::getDeviceIdentifierString(id_string type, char id[17])
{
  uint8_t cmd[2];
  uint8_t rep[20];
...
void 
Microstrain::receiveAccelAngrateMagOrientation (uint64_t *time, double accel[3], double angrate[3], double mag[3], double orientation[9]) 
{
...
void
Microstrain::receiveRawAccelAngrate(uint64_t *time, double accel[3], double angrate[3])
{
  int i, k;
  ...
  receive(Microstrain::CMD_RAW, rep, sizeof(rep), 1000, &sys_time);
...
uint64_t
Microstrain::extractTime(uint8_t* addr)
{
...
int Microstrain::transact(void *cmd, int cmd_len, void *rep, int rep_len, int timeout)
{
  send(cmd, cmd_len);
...
int
Microstrain::send(void *cmd, int cmd_len)
{
  int bytes;
  ...
  if (bytes < 0)
    IMU_EXCEPT(microstrain_3dmgx3_imu::Exception, "error writing to IMU [%s]", strerror(errno));

  if (bytes != cmd_len)
    IMU_EXCEPT(microstrain_3dmgx3_imu::Exception, "whole message not written to IMU");

  // Make sure the queue is drained
  // Synchronous IO doesnt always work
  if (tcdrain(fd) != 0)
    IMU_EXCEPT(microstrain_3dmgx3_imu::Exception, "tcdrain failed");
...
static int read_with_timeout(int fd, void *buff, size_t count, int timeout)
{
  ...
  if ( (retval = poll(ufd, 1, timeout)) < 0 )
    IMU_EXCEPT(microstrain_3dmgx3_imu::Exception, "poll failed  [%s]", strerror(errno));

  if (retval == 0)
    IMU_EXCEPT(microstrain_3dmgx3_imu::TimeoutException, "timeout reached");
	
  nbytes = read(fd, (uint8_t *) buff, count);

  if (nbytes < 0)
    IMU_EXCEPT(microstrain_3dmgx3_imu::Exception, "read failed  [%s]", strerror(errno));
...
// Returns the number of bytes read.
int
Microstrain::receive(uint8_t command, void *rep, int rep_len, int timeout, uint64_t* sys_time)
{
  ...
  *(uint8_t*)(rep) = 0;
  ...
  if (checksum != bswap_16(*(uint16_t*)((uint8_t*)rep+rep_len-2)))
    IMU_EXCEPT(microstrain_3dmgx3_imu::CorruptedDataException, "invalid checksum.\n Make sure the IMU sensor is connected to this computer.");
...
uint64_t Microstrain::filterTime(uint64_t imu_time, uint64_t sys_time)
{
...
double Microstrain::toDouble(uint64_t time)
{
...
uint64_t  Microstrain::toUint64_t(double time)
{
