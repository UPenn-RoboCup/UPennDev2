...
for sm, en in pairs(Config.fsm.enabled) do
	local co = coroutine.wrap(co_fsm)
	state_threads[sm] = co
	state_times[sm] = 0
	-- Initial setup
	local msg = co(sm, en)
	--if not status then print('Bad entry', msg) end
end
...
repeat
  ...
  for name,th in pairs(state_threads) do
		--if coroutine.status(th)~='dead' then
			local t00 = get_time()
			local msg = th(running)
			local t11 = get_time()
			state_times[name] = state_times[name] + (t11-t00)
			--print(name, t11-t00)
			--if not status then print(string.format('%s: %s', util.color(name, 'red'), tostring(msg))) end
		--end
	end
