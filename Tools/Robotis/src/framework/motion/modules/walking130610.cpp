...
Walking130610* Walking130610::m_UniqueInstance = new Walking130610();

static PRO54 *g_pro54 = new PRO54();

Walking130610::Walking130610()
{
	uID = "Walking130610";
	X_OFFSET = 0;
	Y_OFFSET = 75.0;
	...
	BALANCE_HIP_ROLL_GAIN = 0.9;
	BALANCE_ANKLE_ROLL_GAIN = 1.0;
	BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN = 0.0;
	BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN = 0.0;
	...
	FB_BALANCE_ENABLE = false;
	RL_BALANCE_ENABLE = false;

	FB_ALL_BALANCE_ENABLE = false;
	RL_ALL_BALANCE_ENABLE = false;

	m_Euler_Init_Angle_X = 0.0;
	m_Euler_Init_Angle_Y = 0.0;
}
...
void Walking130610::LoadINISettings(minIni* ini)
{
    LoadINISettings(ini, "Walking130610 Config");
}

void Walking130610::LoadINISettings(minIni* ini, const std::string &section)
{
    ...
    if((value = ini->getd(section, "balance_ankle_pitch_euler_angle_gain", INVALID_VALUE)) != INVALID_VALUE) BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN = value;
    if((value = ini->getd(section, "balance_ankle_roll_euler_angle_gain", INVALID_VALUE)) != INVALID_VALUE) BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN = value;
...
void Walking130610::SaveINISettings(minIni* ini)
{
    SaveINISettings(ini, WALKING130610_SECTION);
}
void Walking130610::SaveINISettings(minIni* ini, const std::string &section)
{
    ...
    ini->put(section,   "balance_ankle_pitch_euler_angle_gain",    BALANCE_HIP_ROLL_GAIN);
    ini->put(section,   "balance_ankle_roll_euler_angle_gain",  BALANCE_ANKLE_ROLL_GAIN);
...
double Walking130610::wsin(double time, double period, double period_shift, double mag, double mag_shift)
{
	return mag * sin(2 * 3.141592 / period * time - period_shift) + mag_shift;
}

bool Walking130610::computeIK(double *out, double x, double y, double z, double a, double b, double c)
{
...
void Walking130610::update_param_time()
{
...
void Walking130610::update_param_move()
{
	// Forward/Back
...
void Walking130610::update_param_balance()
{
	m_X_Offset = X_OFFSET;
    m_Y_Offset = Y_OFFSET;
    m_Z_Offset = Z_OFFSET;
...
void Walking130610::Initialize()
{
	...
	for(unsigned int jointIndex = 0; jointIndex < m_RobotInfo.size(); jointIndex ++)
	{
		...
		if(id >= 1 && id <= 26)
		{
			...
			{
				m_RobotInfo[jointIndex].m_Value = m_RobotInfo[jointIndex].m_DXLInfo->Angle2Value(-73.873);
				m_RobotInfo[jointIndex].m_Pgain = 8;
			}
			else if(id == 4)
			{
			...
			{
				m_RobotInfo[jointIndex].m_Value = m_RobotInfo[jointIndex].m_DXLInfo->Angle2Value(29.300);
				m_RobotInfo[jointIndex].m_Pgain = 8;
			}
			else if(id == 8)
			{
				m_RobotInfo[jointIndex].m_Value = m_RobotInfo[jointIndex].m_DXLInfo->Angle2Value(-29.593);
				m_RobotInfo[jointIndex].m_Pgain = 8;
			}
			else if(id == 9)
			{
			...
			{
				m_RobotInfo[jointIndex].m_Value = 0;
				m_RobotInfo[jointIndex].m_Pgain = 32;
			}
			else if(id == 26)
			{
...
void Walking130610::SetInitialEulerAngle()
{

	double dInitAngleX = 0.0;
	double dInitAngleY = 0.0;

	for(int i = 0 ; i < 10 ; i++)
	{
		dInitAngleX += MotionStatus::EulerAngleX;
		dInitAngleY += MotionStatus::EulerAngleY;
		usleep(10000);
	}

	m_Euler_Init_Angle_X = dInitAngleX / 10.0;
	m_Euler_Init_Angle_Y = dInitAngleY / 10.0;

}

void Walking130610::Start()
{
	if(MotionStatus::m_CurrentJoints.size() != 0)
		m_RobotInfo = MotionStatus::m_CurrentJoints;
	else
...
void Walking130610::Stop()
{
	m_Ctrl_Running = false;
}

bool Walking130610::IsRunning()
{
	return m_Real_Running;
}

void Walking130610::Process()
{
    ...
    if(BALANCE_ENABLE == true)
    {
    	...
    	double yAngleErr = m_Euler_Init_Angle_Y - MotionStatus::EulerAngleY ;

		outValue[4] += (int)(dir[4] * yAngleErr *180.0 / PI* BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN*490.23); // R_ANKLE_PITCH
		outValue[10] += (int)(dir[10] * yAngleErr *180.0 / PI* BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN*490.23); // L_ANKLE_PITCH

		double xAngleErr = m_Euler_Init_Angle_X - MotionStatus::EulerAngleX;

		outValue[5] += (int)(dir[5] * xAngleErr *180.0 / PI* BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN*490.23); // R_ANKLE_ROLL
		outValue[11] += (int)(dir[11] * xAngleErr *180.0 / PI* BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN*490.23); // L_ANKLE_ROLL
    ...
    if(FB_BALANCE_ENABLE)
    {
    	double yAngleErr = m_Euler_Init_Angle_Y - MotionStatus::EulerAngleY ;

        outValue[3] += (int)(dir[3] * yAngleErr * 180.0 / PI* BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN/3.0*490.23); // R_KNEE
        outValue[9] += (int)(dir[9] * yAngleErr * 180.0 / PI* BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN/3.0*490.23); // L_KNEE
    ...
    if(RL_BALANCE_ENABLE)
    {
		double xAngleErr = m_Euler_Init_Angle_X - MotionStatus::EulerAngleX;

		outValue[1] -= (int)(dir[1] * xAngleErr * 0.5*180.0 / PI* BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN*490.23); // R_HIP_ROLL
        outValue[7] -= (int)(dir[7] * xAngleErr * 0.5*180.0 / PI* BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN*490.23); // L_HIP_ROLL
