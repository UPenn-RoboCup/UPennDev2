...
local char_to_override_support = {
  ['w'] = vector.new({0.01, 0, 0,   0,0,0,0}),
  ['x'] = vector.new({-.01, 0, 0,   0,0,0,0}),
  ['a'] = vector.new({0, 0.01, 0,   0,0,0,0}),
  ['d'] = vector.new({0, -.01, 0,   0,0,0,0}),
  ['q'] = vector.new({0, 0, 0.01,  0,0,0,0}),
  ['z'] = vector.new({0,0, -.01,   0,0,0,0}),
}
...
local function process_character(key_code,key_char,key_char_lower)
  ...
  local trmod = char_to_override_support[key_char_lower]
  if trmod then
    hcm.set_state_override_support(trmod)    
    return
  end
