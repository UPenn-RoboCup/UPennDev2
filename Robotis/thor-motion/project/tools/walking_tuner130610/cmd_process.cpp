...
{	
	...
	for(unsigned int jointIndex = 0; jointIndex < MotionStatus::m_CurrentJoints.size(); jointIndex++)
	{
		...
		if(id == 25 || id == 26 || id == 27 || id  == 28)
		{
			...
			//Walking130610::GetInstance()->m_RobotInfo[jointIndex].m_Value = 0;
		}
	}

	Walking130610::GetInstance()->Initialize();
	...
	for(unsigned int jointIndex = 0; jointIndex < MotionStatus::m_CurrentJoints.size(); jointIndex++)
	{
		...
		id = MotionStatus::m_CurrentJoints[jointIndex].m_ID;
		wStartPosition = MotionStatus::m_CurrentJoints[jointIndex].m_Value;
		wGoalPosition = Walking130610::GetInstance()->m_RobotInfo[jointIndex].m_Value;
	...
	system("clear");
	printf("\n");
	printf("[Walking130610 Tuner for Thor %s]\n", PROGRAM_VERSION);
	printf("\n");
...
void DrawEnding()
{
	system("clear");
	printf("\n");
	printf("Terminate Walking130610 Tuner");
	printf("\n");
}
...
void DrawScreen()
{
	...
	printf("Balance ankle p ea gain   \n"); // 5
	printf("Balance ankle r ea gain   \n"); // 6
    printf("P gain                    \n"); // 7
    printf("I gain                    \n"); // 8
    printf("D gain                    \n"); // 9
    ClearCmd(); // 8
	...
	if(Walking130610::GetInstance()->IsRunning() == true)
		printf("ON     ");
	else
		printf("OFF    ");

	GoToCursor(PARAM_COL, X_OFFSET_ROW);
	printf("%d    ", (int)Walking130610::GetInstance()->X_OFFSET);

	GoToCursor(PARAM_COL, Y_OFFSET_ROW);
	printf("%d    ", (int)Walking130610::GetInstance()->Y_OFFSET);

	GoToCursor(PARAM_COL, Z_OFFSET_ROW);
	printf("%d    ", (int)Walking130610::GetInstance()->Z_OFFSET);

	GoToCursor(PARAM_COL, ROLL_OFFSET_ROW);
	printf("%.1f    ", Walking130610::GetInstance()->R_OFFSET);

	GoToCursor(PARAM_COL, PITCH_OFFSET_ROW);
	printf("%.1f    ", Walking130610::GetInstance()->P_OFFSET);

	GoToCursor(PARAM_COL, YAW_OFFSET_ROW);
	printf("%.1f    ", Walking130610::GetInstance()->A_OFFSET);

	GoToCursor(PARAM_COL, HIP_PITCH_OFFSET_ROW);
	printf("%.1f    ", Walking130610::GetInstance()->HIP_PITCH_OFFSET);

	GoToCursor(PARAM_COL, AUTO_BALANCE_ROW);
	if(Walking130610::GetInstance()->BALANCE_ENABLE == true)
		printf("ON     ");
	else
		printf("OFF    ");

	GoToCursor(PARAM_COL, PERIOD_TIME_ROW);
	printf("%d    ", (int)Walking130610::GetInstance()->PERIOD_TIME);

	GoToCursor(PARAM_COL, DSP_RATIO_ROW);
	printf("%.2f    ", Walking130610::GetInstance()->DSP_RATIO);

    GoToCursor(PARAM_COL, STEP_FORWARDBACK_RATIO_ROW);
    printf("%.2f    ", Walking130610::GetInstance()->STEP_FB_RATIO);

	GoToCursor(PARAM_COL, STEP_FORWARDBACK_ROW);
	printf("%d    ", (int)Walking130610::GetInstance()->X_MOVE_AMPLITUDE);

	GoToCursor(PARAM_COL, STEP_RIGHTLEFT_ROW);
	printf("%d    ", (int)Walking130610::GetInstance()->Y_MOVE_AMPLITUDE);

	GoToCursor(PARAM_COL, STEP_DIRECTION_ROW);
	printf("%d    ", (int)Walking130610::GetInstance()->A_MOVE_AMPLITUDE);

	GoToCursor(PARAM_COL, TURNING_AIM_ROW);
	if(Walking130610::GetInstance()->A_MOVE_AIM_ON == true)
		printf("ON     ");
	else
		printf("OFF    ");

	GoToCursor(PARAM_COL, FOOT_HEIGHT_ROW);
	printf("%d    ", (int)Walking130610::GetInstance()->Z_MOVE_AMPLITUDE);

	GoToCursor(PARAM_COL, SWING_RIGHTLEFT_ROW);
	printf("%.1f    ", Walking130610::GetInstance()->Y_SWAP_AMPLITUDE);

	GoToCursor(PARAM_COL, SWING_TOPDOWN_ROW);
	printf("%d    ", (int)Walking130610::GetInstance()->Z_SWAP_AMPLITUDE);

	GoToCursor(PARAM_COL, PELVIS_OFFSET_ROW);
	printf("%.1f    ", Walking130610::GetInstance()->PELVIS_OFFSET);

	GoToCursor(PARAM_COL, ARM_SWING_GAIN_ROW);
	printf("%.1f    ", Walking130610::GetInstance()->ARM_SWING_GAIN);

	GoToCursor(PARAM_COL, BAL_KNEE_GAIN_ROW);
	printf("%.2f    ", Walking130610::GetInstance()->BALANCE_KNEE_GAIN);

	GoToCursor(PARAM_COL, BAL_ANKLE_PITCH_GAIN_ROW);
	printf("%.2f    ", Walking130610::GetInstance()->BALANCE_ANKLE_PITCH_GAIN);

	GoToCursor(PARAM_COL, BAL_HIP_ROLL_GAIN_ROW);
	printf("%.2f    ", Walking130610::GetInstance()->BALANCE_HIP_ROLL_GAIN);

	GoToCursor(PARAM_COL, BAL_ANKLE_ROLL_GAIN_ROW);
	printf("%.2f    ", Walking130610::GetInstance()->BALANCE_ANKLE_ROLL_GAIN);

	GoToCursor(PARAM_COL, BAL_ANKLE_PITCH_EULER_ANGLE_GAIN);
	printf("%.2f    ", Walking130610::GetInstance()->BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN);

	GoToCursor(PARAM_COL, BAL_ANKLE_ROLL_EULER_ANGLE_GAIN);
	printf("%.2f    ", Walking130610::GetInstance()->BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN);

    GoToCursor(PARAM_COL, P_GAIN_ROW);
    printf("%d    ", Walking130610::GetInstance()->P_GAIN);

    GoToCursor(PARAM_COL, I_GAIN_ROW);
    printf("%d    ", Walking130610::GetInstance()->I_GAIN);

    GoToCursor(PARAM_COL, D_GAIN_ROW);
    printf("%d    ", Walking130610::GetInstance()->D_GAIN);
...
void IncreaseValue(bool large)
{
	...
	case WALKING_MODE_ROW:
		...
		Walking130610::GetInstance()->Start();
		printf("ON    ");
		break;

	case X_OFFSET_ROW:
		if(large == true)
			Walking130610::GetInstance()->X_OFFSET += 10;
		else
			Walking130610::GetInstance()->X_OFFSET += 1;
		printf("%d    ", (int)Walking130610::GetInstance()->X_OFFSET);
		break;

	case Y_OFFSET_ROW:
		if(large == true)
			Walking130610::GetInstance()->Y_OFFSET += 10;
		else
			Walking130610::GetInstance()->Y_OFFSET += 1;
		printf("%d    ", (int)Walking130610::GetInstance()->Y_OFFSET);
		break;

	case Z_OFFSET_ROW:
		if(large == true)
			Walking130610::GetInstance()->Z_OFFSET += 10;
		else
			Walking130610::GetInstance()->Z_OFFSET += 1;
		printf("%d    ", (int)Walking130610::GetInstance()->Z_OFFSET);
		break;

	case ROLL_OFFSET_ROW:
		if(large == true)
			Walking130610::GetInstance()->R_OFFSET += 1.0;
		else
			Walking130610::GetInstance()->R_OFFSET += 0.1;
		printf("%.1f    ", Walking130610::GetInstance()->R_OFFSET);
		break;

	case PITCH_OFFSET_ROW:
		if(large == true)
			Walking130610::GetInstance()->P_OFFSET += 1.0;
		else
			Walking130610::GetInstance()->P_OFFSET += 0.1;
		printf("%.1f    ", Walking130610::GetInstance()->P_OFFSET);
		break;

	case YAW_OFFSET_ROW:
		if(large == true)
			Walking130610::GetInstance()->Y_OFFSET += 1.0;
		else
			Walking130610::GetInstance()->Y_OFFSET += 0.1;
		printf("%.1f    ", Walking130610::GetInstance()->Y_OFFSET);
		break;

	case HIP_PITCH_OFFSET_ROW:
		if(large == true)
			Walking130610::GetInstance()->HIP_PITCH_OFFSET += 1.0;
		else
			Walking130610::GetInstance()->HIP_PITCH_OFFSET += 0.1;
		printf("%.1f    ", Walking130610::GetInstance()->HIP_PITCH_OFFSET);
		break;

	case AUTO_BALANCE_ROW:
		Walking130610::GetInstance()->BALANCE_ENABLE = true;
		printf("ON    ");
		break;

	case PERIOD_TIME_ROW:
		if(large == true)
			Walking130610::GetInstance()->PERIOD_TIME += 10;
		else
			Walking130610::GetInstance()->PERIOD_TIME += 1;
		printf("%d    ", (int)Walking130610::GetInstance()->PERIOD_TIME);
		break;

	case DSP_RATIO_ROW:
		if(large == true)
			Walking130610::GetInstance()->DSP_RATIO += 0.1;
		else
			Walking130610::GetInstance()->DSP_RATIO += 0.01;
		printf("%.2f    ", Walking130610::GetInstance()->DSP_RATIO);
		break;

    case STEP_FORWARDBACK_RATIO_ROW:
        if(large == true)
            Walking130610::GetInstance()->STEP_FB_RATIO += 0.1;
        else
            Walking130610::GetInstance()->STEP_FB_RATIO += 0.01;
        printf("%.2f    ", Walking130610::GetInstance()->STEP_FB_RATIO);
        break;

	case STEP_FORWARDBACK_ROW:
		if(large == true)
			Walking130610::GetInstance()->X_MOVE_AMPLITUDE += 10;
		else
			Walking130610::GetInstance()->X_MOVE_AMPLITUDE += 1;
		printf("%d    ", (int)Walking130610::GetInstance()->X_MOVE_AMPLITUDE);
		break;

	case STEP_RIGHTLEFT_ROW:
		if(large == true)
			Walking130610::GetInstance()->Y_MOVE_AMPLITUDE += 10;
		else
			Walking130610::GetInstance()->Y_MOVE_AMPLITUDE += 1;
		printf("%d    ", (int)Walking130610::GetInstance()->Y_MOVE_AMPLITUDE);
		break;

	case STEP_DIRECTION_ROW:
		if(large == true)
			Walking130610::GetInstance()->A_MOVE_AMPLITUDE += 10;
		else
			Walking130610::GetInstance()->A_MOVE_AMPLITUDE += 1;
		printf("%d    ", (int)Walking130610::GetInstance()->A_MOVE_AMPLITUDE);
		break;

	case TURNING_AIM_ROW:
		Walking130610::GetInstance()->A_MOVE_AIM_ON = true;
		printf("ON   ");
		break;

	case FOOT_HEIGHT_ROW:
		if(large == true)
			Walking130610::GetInstance()->Z_MOVE_AMPLITUDE += 10;
		else
			Walking130610::GetInstance()->Z_MOVE_AMPLITUDE += 1;
		printf("%d    ", (int)Walking130610::GetInstance()->Z_MOVE_AMPLITUDE);
		break;

	case SWING_RIGHTLEFT_ROW:
		if(large == true)
			Walking130610::GetInstance()->Y_SWAP_AMPLITUDE += 1.0;
		else
			Walking130610::GetInstance()->Y_SWAP_AMPLITUDE += 0.1;
		printf("%.1f    ", Walking130610::GetInstance()->Y_SWAP_AMPLITUDE);
		break;

	case SWING_TOPDOWN_ROW:
		if(large == true)
			Walking130610::GetInstance()->Z_SWAP_AMPLITUDE += 10;
		else
			Walking130610::GetInstance()->Z_SWAP_AMPLITUDE += 1;
		printf("%d    ", (int)Walking130610::GetInstance()->Z_SWAP_AMPLITUDE);
		break;

	case PELVIS_OFFSET_ROW:
		if(large == true)
			Walking130610::GetInstance()->PELVIS_OFFSET += 1.0;
		else
			Walking130610::GetInstance()->PELVIS_OFFSET += 0.1;
		printf("%.1f    ", Walking130610::GetInstance()->PELVIS_OFFSET);
		break;

	case ARM_SWING_GAIN_ROW:
		if(large == true)
			Walking130610::GetInstance()->ARM_SWING_GAIN += 0.10;
		else
			Walking130610::GetInstance()->ARM_SWING_GAIN += 0.01;
		printf("%.1f    ", Walking130610::GetInstance()->ARM_SWING_GAIN);
		break;

	case BAL_KNEE_GAIN_ROW:
		if(large == true)
			Walking130610::GetInstance()->BALANCE_KNEE_GAIN += 0.1;
		else
			Walking130610::GetInstance()->BALANCE_KNEE_GAIN += 0.01;
		printf("%.2f    ", Walking130610::GetInstance()->BALANCE_KNEE_GAIN);
		break;

	case BAL_ANKLE_PITCH_GAIN_ROW:
		if(large == true)
			Walking130610::GetInstance()->BALANCE_ANKLE_PITCH_GAIN += 0.1;
		else
			Walking130610::GetInstance()->BALANCE_ANKLE_PITCH_GAIN += 0.01;
		printf("%.2f    ", Walking130610::GetInstance()->BALANCE_ANKLE_PITCH_GAIN);
		break;

	case BAL_HIP_ROLL_GAIN_ROW:
		if(large == true)
			Walking130610::GetInstance()->BALANCE_HIP_ROLL_GAIN += 0.1;
		else
			Walking130610::GetInstance()->BALANCE_HIP_ROLL_GAIN += 0.01;
		printf("%.2f    ", Walking130610::GetInstance()->BALANCE_HIP_ROLL_GAIN);
		break;

	case BAL_ANKLE_ROLL_GAIN_ROW:
		if(large == true)
			Walking130610::GetInstance()->BALANCE_ANKLE_ROLL_GAIN += 0.1;
		else
			Walking130610::GetInstance()->BALANCE_ANKLE_ROLL_GAIN += 0.01;
		printf("%.2f    ", Walking130610::GetInstance()->BALANCE_ANKLE_ROLL_GAIN);
		break;

	case BAL_ANKLE_PITCH_EULER_ANGLE_GAIN:
		if(large == true)
			Walking130610::GetInstance()->BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN += 0.1;
		else
			Walking130610::GetInstance()->BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN += 0.01;
		printf("%.2f    ", Walking130610::GetInstance()->BALANCE_ANKLE_PITCH_EULER_ANGLE_GAIN);
		break;

	case BAL_ANKLE_ROLL_EULER_ANGLE_GAIN:
		if(large == true)
			Walking130610::GetInstance()->BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN += 0.1;
		else
			Walking130610::GetInstance()->BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN += 0.01;
		printf("%.2f    ", Walking130610::GetInstance()->BALANCE_ANKLE_ROLL_EULER_ANGLE_GAIN);
		break;

    case P_GAIN_ROW:
        if(large == true)
            Walking130610::GetInstance()->P_GAIN += 10;
        else
            Walking130610::GetInstance()->P_GAIN += 1;
        printf("%d    ", Walking130610::GetInstance()->P_GAIN);
        break;

    case I_GAIN_ROW:
        if(large == true)
            Walking130610::GetInstance()->I_GAIN += 10;
        else
            Walking130610::GetInstance()->I_GAIN += 1;
        printf("%d    ", Walking130610::GetInstance()->I_GAIN);
        break;

    case D_GAIN_ROW:
        if(large == true)
            Walking130610::GetInstance()->D_GAIN += 10;
        else
            Walking130610::GetInstance()->D_GAIN += 1;
        printf("%d    ", Walking130610::GetInstance()->D_GAIN);
        break;
	}
...
void DecreaseValue(bool large)
{
	...
	{
	case WALKING_MODE_ROW:
		Walking130610::GetInstance()->Stop();
		MotionManager::GetInstance()->StopLogging();
		printf("OFF");
		GoToCursor(PARAM_COL, STEP_FORWARDBACK_ROW);
		Walking130610::GetInstance()->X_MOVE_AMPLITUDE = 0;
		printf("%d    ", (int)Walking130610::GetInstance()->X_MOVE_AMPLITUDE);
		GoToCursor(PARAM_COL, STEP_RIGHTLEFT_ROW);
		Walking130610::GetInstance()->Y_MOVE_AMPLITUDE = 0;
		printf("%d    ", (int)Walking130610::GetInstance()->Y_MOVE_AMPLITUDE);
		GoToCursor(PARAM_COL, STEP_DIRECTION_ROW);
		Walking130610::GetInstance()->A_MOVE_AMPLITUDE = 0;
		printf("%.1f    ", Walking130610::GetInstance()->A_MOVE_AMPLITUDE);
		break;
	...
	case TURNING_AIM_ROW:
		Walking130610::GetInstance()->A_MOVE_AIM_ON = false;
		printf("OFF   ");
		break;
	...
	case ARM_SWING_GAIN_ROW:
		if(large == true)
			Walking130610::GetInstance()->ARM_SWING_GAIN -= 1.0;
		else
			Walking130610::GetInstance()->ARM_SWING_GAIN -= 0.1;
		printf("%.1f    ", Walking130610::GetInstance()->ARM_SWING_GAIN);
		break;
