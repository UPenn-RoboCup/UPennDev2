...
shared.head_camera.net = vector.new({2,1,30,1})
...
-- Quality | JPEG quality from 0-100
-- Sending Rate | Number of seconds between frames
shared.forehead_camera.net = vector.new({2,1,50,2})

-- Forehead Camera --
shared.forehead2_camera     = {}
shared.forehead2_camera.t   = vector.zeros(1)
-- Network Requests: [stream,compression,quality,fps]
-- Stream | 0: None, 1: Single Frame, 2: Stream, 3: Single Reliable Frame 4: Reliable Stream
-- Compression | 0: None, 1: JPEG, 3: PNG
-- Quality | JPEG quality from 0-100
...
shared.chest_lidar.scanlines = vector.new({-45*DEG_TO_RAD,45*DEG_TO_RAD,3/DEG_TO_RAD})
-- Care only about lidar readings within this field of view
-- {Start angle, stop angle}
--shared.chest_lidar.fov      = vector.new({-40,80})*DEG_TO_RAD
--shared.chest_lidar.fov      = vector.new({-60,80})*DEG_TO_RAD
--shared.chest_lidar.fov      = vector.new({-45,60})*DEG_TO_RAD
shared.chest_lidar.fov      = vector.new({-30,60})*DEG_TO_RAD
