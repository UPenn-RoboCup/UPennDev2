...
pushers_tbl['lidar'] = function ( lidar_tbl )
  local encoded_lidar = mp.pack(lidar_tbl)
  --assert(
  ipc_channels['lidar']:send(encoded_lidar)
  --,print('Lidar push') );
end
pushers_tbl['arduimu'] = function ( imu_tbl )
  local encoded_imu = mp.pack(imu_tbl)
  --assert( 
  ipc_channels['arduimu']:send(encoded_imu)
  --,print('IMU push') );
end
pushers_tbl['flir'] = function ( flir_tbl )
  local encoded_flir = mp.pack(flir_tbl)
  --assert( 
  ipc_channels['flir']:send(encoded_flir)
  --,print('Flir push') );
end
...
while true do
  ...
  local min_ts, d_idx = min(entry_timestamps)
