...
static int forward_joints(lua_State *L)
{
  ...
  std::vector<double> q = saffir_kinematics_forward_joints(&r[0]);
  lua_pushvector(L, q);
  return 1;
...
static int forward_head(lua_State *L) {
  ...
  Transform t = saffir_kinematics_forward_head(&q[0]);
...
static int forward_l_arm(lua_State *L) {
  std::vector<double> q = lua_checkvector(L, 1);
  Transform t = saffir_kinematics_forward_l_arm(&q[0]);
  lua_pushtransform(L, t);
  return 1;
}
...
static int forward_r_arm(lua_State *L) {
  std::vector<double> q = lua_checkvector(L, 1);
  Transform t = saffir_kinematics_forward_r_arm(&q[0]);
  lua_pushtransform(L, t);
  return 1;
}
...
static int forward_l_leg(lua_State *L) {
  std::vector<double> q = lua_checkvector(L, 1);
  Transform t = saffir_kinematics_forward_l_leg(&q[0]);
  lua_pushtransform(L, t);
  return 1;
}
...
static int forward_r_leg(lua_State *L) {
  std::vector<double> q = lua_checkvector(L, 1);
  Transform t = saffir_kinematics_forward_r_leg(&q[0]);
  lua_pushtransform(L, t);
  return 1;
}
...
static int torso_l_arm(lua_State *L) {
  std::vector<double> q = lua_checkvector(L, 1);
  Transform t = inv(saffir_kinematics_forward_l_arm(&q[0]));
  lua_pushvector(L, position6D(t));
  return 1;
}
...
static int torso_r_arm(lua_State *L) {
  std::vector<double> q = lua_checkvector(L, 1);
  Transform t = inv(saffir_kinematics_forward_r_arm(&q[0]));
  lua_pushvector(L, position6D(t));
  return 1;
}
...
static int torso_l_leg(lua_State *L) {
  std::vector<double> q = lua_checkvector(L, 1);
  Transform t = inv(saffir_kinematics_forward_l_leg(&q[0]));
  lua_pushvector(L, position6D(t));
  return 1;
}
...
static int torso_r_leg(lua_State *L) {
  std::vector<double> q = lua_checkvector(L, 1);
  Transform t = inv(saffir_kinematics_forward_r_leg(&q[0]));
  lua_pushvector(L, position6D(t));
  return 1;
}
...
static int inverse_joints(lua_State *L)
{
  ...
  std::vector<double> q = lua_checkvector(L, 1);
  std::vector<double> r = saffir_kinematics_inverse_joints(&q[0]);
  lua_pushvector(L, r);
  return 1;
}
...
static int inverse_l_arm(lua_State *L) {
  ...
  qArm = saffir_kinematics_inverse_r_arm(trArm);
...
static int inverse_r_arm(lua_State *L) {
  ...
  std::vector<double> pArm = lua_checkvector(L, 1);
  Transform trArm = transform6D(&pArm[0]);
  qArm = saffir_kinematics_inverse_l_arm(trArm);
  lua_pushvector(L, qArm);
  return 1;
}
...
static int inverse_arms(lua_State *L) {
  ...
  qLArm = saffir_kinematics_inverse_l_arm(trTorso_LArm);
  qRArm = saffir_kinematics_inverse_r_arm(trTorso_RArm);
...
static int inverse_l_leg(lua_State *L) {
  ...
  qLeg = saffir_kinematics_inverse_r_leg(trLeg);
...
static int inverse_r_leg(lua_State *L) {
  ...
  std::vector<double> pLeg = lua_checkvector(L, 1);
  Transform trLeg = transform6D(&pLeg[0]);
  qLeg = saffir_kinematics_inverse_l_leg(trLeg);
  lua_pushvector(L, qLeg);
  return 1;
}

static int saffir_inverse_legs(lua_State *L) {
  ...
  qLLeg = saffir_kinematics_inverse_l_leg(trTorso_LLeg);
  qRLeg = saffir_kinematics_inverse_r_leg(trTorso_RLeg);
...
static const struct luaL_reg kinematics_lib [] = {
  ...
  {"inverse_legs", saffir_inverse_legs},
