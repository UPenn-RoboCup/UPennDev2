...
function state.entry()
  ...
  plan_valid,stage = arm_planner:plan_arm_sequence(Config.armfsm.teleopr.arminit,stage,"arminit")  
end
...
function state.update()
  ...
  elseif stage=="teleopwait" then          
    if hcm.get_state_proceed(0)== -1 then       
      plan_valid,stage = arm_planner:plan_arm_sequence(Config.armfsm.teleopr.armuninit,stage,"armuninit")        
      if not plan_valid then
        plan_valid=true
