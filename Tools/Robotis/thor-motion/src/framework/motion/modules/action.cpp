...
void Action::ResetPage(PAGE *pPage)
{
	...
	for(int i=0; i < MotionStatus::MAXIMUM_NUMBER_OF_JOINTS; i++)
		pPage->header.pgain[i] = 0x55;

    for(int i=0; i<MAXNUM_STEP; i++)
    {
        for(int j=0; j < MotionStatus::MAXIMUM_NUMBER_OF_JOINTS; j++)
...
bool Action::Start(int index, PAGE *pPage)
{
	...
	for(unsigned int jointIndex = 0; jointIndex < MotionStatus::m_CurrentJoints.size() ; jointIndex++)
		MotionStatus::m_EnableList[(MotionStatus::m_CurrentJoints[jointIndex].m_ID) - 1].uID = "Action";
...
bool Action::Start(int iPage, EnableList *enable_List)
{
	if( iPage < 1 || iPage >= MAXNUM_PAGE )
	{
		if(DEBUG_PRINT == true)
	...
	return Start(iPage, &page, enable_List);
}

bool Action::Start(char* namePage, EnableList *enable_List)
{
	int index;
	PAGE page;
	...
	return Start(index, &page, enable_List);
}

bool Action::Start(int index, PAGE *pPage, EnableList *enable_List)
{
	if(m_Playing == true)
	{
		if(DEBUG_PRINT == true)
	...
	for(unsigned int jointIndex = 0; jointIndex < MotionStatus::m_CurrentJoints.size() ; jointIndex++)
		MotionStatus::m_EnableList[(MotionStatus::m_CurrentJoints[jointIndex].m_ID) - 1].uID = enable_List[(MotionStatus::m_CurrentJoints[jointIndex].m_ID)-1].uID;
...
void Action::Process()
{
    ...
    ///////////////// Static 변수
	static unsigned short wpStartAngle1024[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS]; // 보간할 시작 지점
    static unsigned short wpTargetAngle1024[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS]; // 보간할 도착 지점
    static short int ipMovingAngle1024[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS]; // 총 가야할 거리
    static short int ipMainAngle1024[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS]; // 등속 구간에서 가야할 거리
    static short int ipAccelAngle1024[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS]; // 가속 구간에서 가야할 거리
    static short int ipMainSpeed1024[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS]; // 목표 등속도
    static short int ipLastOutSpeed1024[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS]; // 이 전 상태의 속도(관성)
    static short int ipGoalSpeed1024[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS]; // 모터가 내야 할 목표속도
    static unsigned char bpFinishType[MotionStatus::MAXIMUM_NUMBER_OF_JOINTS]; // 도착 지점에 도달할 상태
    short int iSpeedN;
    ...
    ***************************************/
    ...
    if( m_FirstDrivingStart == true ) // 처음 시작할때
    {
		...
		for(unsigned int jointIndex = 0; jointIndex<MotionStatus::m_CurrentJoints.size(); jointIndex++)
		{
			bID = MotionStatus::m_CurrentJoints[jointIndex].m_ID;
			wpTargetAngle1024[bID] = MotionStatus::m_CurrentJoints[jointIndex].m_DXLInfo->ValueScaleDown(MotionStatus::m_CurrentJoints[jointIndex].m_Value);
    ...
    if( wUnitTimeCount < wUnitTimeNum ) // 현재 진행중이라면
    {
        ...
        {
            for(unsigned int jointCount = 0; jointCount<m_RobotInfo.size(); jointCount++ )
            {
				bID = m_RobotInfo[jointCount].m_ID;
				if( ipMovingAngle1024[bID] == 0 )
				{
					m_RobotInfo[jointCount].m_Value
						= m_RobotInfo[jointCount].m_DXLInfo->ValueScaleUp(wpStartAngle1024[bID]);
					m_RobotInfo[jointCount].m_Angle
						= m_RobotInfo[jointCount].m_DXLInfo->Value2Angle(m_RobotInfo[jointCount].m_Value);
				}
				else
				{
					if( bSection == PRE_SECTION )
					{
						...
						m_RobotInfo[jointCount].m_Value
							= m_RobotInfo[jointCount].m_DXLInfo->ValueScaleUp(wpStartAngle1024[bID] + ipAccelAngle1024[bID]);
						m_RobotInfo[jointCount].m_Angle
							= m_RobotInfo[jointCount].m_DXLInfo->Value2Angle(m_RobotInfo[jointCount].m_Value);
					}
					...
					{
						m_RobotInfo[jointCount].m_Value
							= m_RobotInfo[jointCount].m_DXLInfo->ValueScaleUp(wpStartAngle1024[bID] + (short int)(((long)(ipMainAngle1024[bID])*wUnitTimeCount) / wUnitTimeNum));
						m_RobotInfo[jointCount].m_Angle
							= m_RobotInfo[jointCount].m_DXLInfo->Value2Angle(m_RobotInfo[jointCount].m_Value);
					...
					else // POST_SECTION
					{
						if( wUnitTimeCount == (wUnitTimeNum-1) )
						{
							...
							m_RobotInfo[jointCount].m_Value
								= m_RobotInfo[jointCount].m_DXLInfo->ValueScaleUp(wpTargetAngle1024[bID]);
							m_RobotInfo[jointCount].m_Angle
								= m_RobotInfo[jointCount].m_DXLInfo->Value2Angle(m_RobotInfo[jointCount].m_Value);
						}
						else
						{
							if( bpFinishType[bID] == ZERO_FINISH )
							{
								...
								m_RobotInfo[jointCount].m_Value
									= m_RobotInfo[jointCount].m_DXLInfo->ValueScaleUp(wpStartAngle1024[bID] +  (short)((((long)(ipLastOutSpeed1024[bID] + (iSpeedN>>1)) * wUnitTimeCount * 144) / 15) >> 9));
								m_RobotInfo[jointCount].m_Angle
									= m_RobotInfo[jointCount].m_DXLInfo->Value2Angle(m_RobotInfo[jointCount].m_Value);
							}
				...
				m_RobotInfo[jointCount].m_Pgain = ( 256 >> (m_PlayPage.header.pgain[bID] >> 4) ) << 2 ;
			}
		}
	}
    else if( wUnitTimeCount >= wUnitTimeNum ) // 현재 Section이 완료되었다면
    {
        ...
        for(unsigned int jointCount = 0; jointCount < MotionStatus::m_CurrentJoints.size(); jointCount++)
        {
			bID = m_RobotInfo[jointCount].m_ID;

			wpStartAngle1024[bID]
				= m_RobotInfo[jointCount].m_DXLInfo->ValueScaleDown(m_RobotInfo[jointCount].m_Value);
        ...
        else if( bSection == PAUSE_SECTION )
        {
            ...
            for(unsigned int jointIndex = 0; jointIndex < MotionStatus::m_CurrentJoints.size(); jointIndex++)
			{
				bID = m_RobotInfo[jointIndex].m_ID;
				ipLastOutSpeed1024[bID] = 0;
			}
        }
