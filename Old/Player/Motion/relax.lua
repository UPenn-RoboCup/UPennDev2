...
function entry()
  ...
  t0 = Body.get_time();
  if(Config.PLATFORM_NAME == 'OP') then
...
--    Body.set_rleg_command({0,0,0,0,knee_pitch_target,0,0});
  ...
  elseif Config.PLATFORM_NAME=='Nao' then
...
function update()
  ...
  if Config.PLATFORM_NAME == 'Nao' then
  ...
  mcm.set_walk_bodyHeight(pTorso[3]);
  mcm.set_walk_bodyTilt(pTorso[5]);
