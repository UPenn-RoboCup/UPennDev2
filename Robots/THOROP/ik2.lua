...
local function ik(trArm, shoulderYaw, FLIP_SHOULDER_ROLL)
  -- TODO: Use the cdata directly, rather than in and out of C
  local xWrist = ffi.new('double[3]', T.get_pos(trArm))
  local dWrist = xWrist[0]*xWrist[0]+xWrist[1]*xWrist[1]+xWrist[2]*xWrist[2]
  local cElbow = .5*(dWrist-dUpperArm*dUpperArm-dLowerArm*dLowerArm)/(dUpperArm*dLowerArm);
  ...
  local m = T.rotX(shoulderYaw)
    ...
    * T.rotY(elbowPitch)
    * T.translateZ(lowerArmLength,0,-elbowOffsetX)
  ...
  local c1 = (m(2,3)*xWrist[2]-t1*xWrist[0]) /(m(2,3)*m(2,3) + t1*t1);
  local s1 = (m(2,3)*xWrist[0]+t1*xWrist[2]) /(m(2,3)*m(2,3) + t1*t1);
  ...
  local err_a = fmodf(qOrg[4] - wristYaw_a+5*PI, 2*PI) - PI;
  local err_b = fmodf(qOrg[4] - wristYaw_b+5*PI, 2*PI) - PI;
