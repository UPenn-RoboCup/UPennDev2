...
int 
RRTstar::Planner<State, Trajectory, System>
::iteration (double* ostate, double* pstate) {
    
    // 1. Sample a new state
    State stateRandom;
    system->sampleState (stateRandom);
    
    int nDim = stateRandom.getNumDimensions();
    
    // Output for debugging
    if(ostate!=NULL){
      for(int i=0;i<nDim;i++){
        ostate[i] = stateRandom[i];
      }
    }
...
::getBestTrajectory (std::list<double*>& trajectoryOut) {
    ...
    while (vertexCurr) {
        ...
        double *stateArrCurr = new double[nDim];
        for(int i=0;i<nDim;i++){
        ...
        if(vertexCurr->hasParent()){
            ...
            for (std::list<double*>::iterator iter = trajectory.begin(); iter != trajectory.end(); ++iter) {
                ...
                /*
                stateArrCurr = new double[nDim];
                for(int i=0;i<nDim;i++){
                  stateArrCurr[i] = stateArrFromParentCurr[i];
                  printf(" %.2f", stateArrCurr[i]);
                }
                printf("\n");
