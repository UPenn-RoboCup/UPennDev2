...
void arm_init() {
  ...
  wb_servo_set_velocity(arm_elements[ARM2], 0.5);
...
void arm_reset() {
  wb_servo_set_position(arm_elements[ARM1], 0.0);
  wb_servo_set_position(arm_elements[ARM2], 1.57);
  wb_servo_set_position(arm_elements[ARM3], -2.64);
  wb_servo_set_position(arm_elements[ARM4], 1.78);
  wb_servo_set_position(arm_elements[ARM5], 0.0);
}

void arm_set_height(enum Height height) {
  switch (height) {
    case ARM_FRONT_FLOOR:
      wb_servo_set_position(arm_elements[ARM2], -0.97);
      wb_servo_set_position(arm_elements[ARM3], -1.55);
      wb_servo_set_position(arm_elements[ARM4], -0.61);
      wb_servo_set_position(arm_elements[ARM5],  0.0);
      break;
    case ARM_FRONT_PLATE:
      wb_servo_set_position(arm_elements[ARM2], -0.62);
      wb_servo_set_position(arm_elements[ARM3], -0.98);
      wb_servo_set_position(arm_elements[ARM4], -1.53);
      wb_servo_set_position(arm_elements[ARM5],  0.0);
      break;
    case ARM_FRONT_CARDBOARD_BOX:
      wb_servo_set_position(arm_elements[ARM2],  0.0);
      wb_servo_set_position(arm_elements[ARM3], -0.77);
      wb_servo_set_position(arm_elements[ARM4], -1.21);
      wb_servo_set_position(arm_elements[ARM5],  0.0);
      break;
    case ARM_RESET:
      wb_servo_set_position(arm_elements[ARM2],  1.57);
    ...
    case ARM_BACK_PLATE_HIGH:
      wb_servo_set_position(arm_elements[ARM2],  0.678);
      wb_servo_set_position(arm_elements[ARM3],  0.682);
      wb_servo_set_position(arm_elements[ARM4],  1.74);
      wb_servo_set_position(arm_elements[ARM5],  0.0);
      break;
    case ARM_BACK_PLATE_LOW:
      wb_servo_set_position(arm_elements[ARM2], 0.92);
      wb_servo_set_position(arm_elements[ARM3], 0.42);
      wb_servo_set_position(arm_elements[ARM4], 1.78);
      wb_servo_set_position(arm_elements[ARM5],  0.0);
      break;
    case ARM_HANOI_PREPARE:
      wb_servo_set_position(arm_elements[ARM2], -0.4);
      wb_servo_set_position(arm_elements[ARM3], -1.2);
      wb_servo_set_position(arm_elements[ARM4], -M_PI_2);
      wb_servo_set_position(arm_elements[ARM5],  M_PI_2);
      break;
    default:
...
void arm_set_orientation(enum Orientation orientation) {
  switch (orientation) {
    case ARM_BACK_LEFT:
      wb_servo_set_position(arm_elements[ARM1], -2.95);
      break;
    case ARM_LEFT:
      wb_servo_set_position(arm_elements[ARM1], -M_PI_2);
      break;
    case ARM_FRONT_LEFT:
      wb_servo_set_position(arm_elements[ARM1], -0.2);
      break;
    case ARM_FRONT:
      wb_servo_set_position(arm_elements[ARM1], 0.0);
      break;
...
void arm_set_sub_arm_rotation( enum Arm arm, double radian) {
  wb_servo_set_position(arm_elements[arm], radian);
}
...
void arm_ik(double x, double y, double z) {
  ...
  wb_servo_set_position(arm_elements[ARM1], alpha);
  wb_servo_set_position(arm_elements[ARM2], beta);
  wb_servo_set_position(arm_elements[ARM3], gamma);
  wb_servo_set_position(arm_elements[ARM4], delta);
  wb_servo_set_position(arm_elements[ARM5], epsilon);
}
