...
libDynamixel.open = function(self, ttyname, ttybaud)
	
	-- Perform require upon an open
	-- TODO: multiple USB2Dynamixel - does this require mess up all?
	unix = require('unix');
	stty = require('stty');
	self.fd = -1;
	self.baud = ttybaud or 1000000;
...
libDynamixel.close = function(self)
	if (fd >= 0) then
		unix.close(self.fd);
	end
	fd = -1;
