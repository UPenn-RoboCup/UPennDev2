...
inline int checksum2(const char *str, int len) {
  int i = 0;
  char sum = 0;
  for (i = 0; i < len - 1; i++) sum += str[i];
  sum = (sum & 0x3f) + 0x30;
...
static int lua_hokuyo_parse3(lua_State *L) {
  size_t packet_len = 0;
  const char * packet = lua_tolstring(L, 1, &packet_len);
  const char * ptr = packet;
  int lf_pos = 0;
...
static int lua_hokuyo_parse2(lua_State *L) {
  size_t packet_len = 0;
  const char * packet = lua_tolstring(L, 1, &packet_len);
  const char * ptr = packet;
  int lf_pos = 0;
...
//#define MAX_COUNT_2 768
//#define MAX_COUNT_CHARS_2 1536
#define MAX_COUNT_2 769
#define MAX_COUNT_CHARS_2 1538
  char raw[MAX_COUNT_CHARS_2];
  char *rawp = &raw[0];
  float scan[MAX_COUNT_2];
  unsigned int raw_scan = 0;
  int scan_idx = 0, j = 0, i = 0;
//printf("ptr: '%s'\n",ptr);
//int lf_pos2 = strfind(ptr, '\n', packet_len);
//printf("%d->%d\n",lf_pos,lf_pos2);

  // TODO: Why 50? Number of 64 byte blocks...
  for (i = 0; i < 23; i++) {
    if (checksum(ptr, LINE_LEN) < 0) {
      printf("%s %d\n", ptr, i);
      printf("Bad checksum line %d\n", i);
      //lua_pushnil(L);
      //return 1;
      ////return luaL_error(L, "Checksum fail: |%s|: |%d|", ptr, i);
    }
  ...
  // TODO: Why 42? Remainder stuff?
  memcpy(rawp, ptr, 50 * sizeof(char));

  for (j = 0; j < MAX_COUNT_CHARS_2; j+=2) {
    //raw_scan = ((raw[j] - 0x30) << 12) + ((raw[j + 1] - 0x30) << 6) + raw[j + 2] - 0x30;
    raw_scan = ((raw[j] - 0x30) << 6) + raw[j + 1] - 0x30;
    scan[scan_idx++] = (float)(raw_scan * 0.001);
  }
  lua_pushlstring(L, (char *)scan, MAX_COUNT_2 * sizeof(float));

  return 1;
}

static const struct luaL_Reg HokuyoPacket_Functions [] = {
  {"parse", lua_hokuyo_parse3},
  {"parse2", lua_hokuyo_parse2},
